...
export default function ArchivesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'archive' | 'export' | 'browse' | 'years' | 'administration'>('overview')
  ...
  const handleExportCurrent = async () => {
    ...
    try {
      const result = await exportFestivalToExcel({ festivalYear: currentYear })
      ...
      <div className="bg-white border-b border-gray-200">
        <nav className="px-6">
          <div className="flex space-x-8">
            {['overview', 'archive', 'export', 'browse', 'years', 'administration'].map((tab) => (
              <button
                key={tab}
